...
import { ProcessingDocumentsTable } from "@/components/documentos/tables/ProcessingDocumentsTable";
import { useProcessingDocuments } from "@/components/documentos/hooks/useProcessingDocuments";
import { useViewDocumentModal } from "@/components/documentos/hooks/useViewDocumentModal";
import { useFlowDiagramModal } from "@/components/documentos/hooks/useFlowDiagramModal";
...
export default function DocumentosPage() {
  ...
  // selectedDocument is now provided by the useViewDocumentModal hook
  ...
  const [isDocumentationModalOpen, setIsDocumentationModalOpen] =
    useState(false);
  const [documentationSelectedDocument, setDocumentationSelectedDocument] = 
  ...
  // Use custom hooks for modals and state management
  const { 
    flowDiagramModal, 
    setFlowDiagramModal,
    showFlowInspector,
  ...
  const {
    ...
    selectedArtifact: viewArtifact,
    openViewModal,
    closeViewModal,
    setSelectedArtifact: setViewArtifact
...
`;
  ...
  const integrateAttachmentsMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (viewDocument?.id) {
        ...
        queryClient.invalidateQueries({
          queryKey: ["/api/documentos", viewDocument.id, "artifacts"],
        });
      }
  ...
  // openViewModal is now provided by the useViewDocumentModal hook
  ...
  const renderDocumentosTable = (documentos: Documento[], showFilters: boolean = true) => {
    return (
      <DocumentosTable
        ...
        setSelectedDocument={setDocumentationSelectedDocument}
