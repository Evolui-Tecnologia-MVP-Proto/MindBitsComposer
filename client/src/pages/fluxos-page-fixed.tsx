...
const FlowCanvas = ({ onFlowInfoChange }: { onFlowInfoChange: (info: {code: string, name: string} | null) => void }) => {
  ...
  const getNodeMetadata = (nodeType: string) => {
    ...
    const currentFlowType = flowTypes.find((ft: any) => ft.id === currentFlow?.flowTypeId);
    if (!currentFlowType?.nodeMetadata?.nodeTypes) return null;
    
    return currentFlowType.nodeMetadata.nodeTypes[nodeType] || null;
  };
