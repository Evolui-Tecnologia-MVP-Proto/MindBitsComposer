...
- **Template Formula REPLACE Function**: Added support for REPLACE() function in template formulas alongside existing SUBSTR() support. Formulas like `REPLACE(sistema, ' ', '') + '-' + responsavel` now properly execute string replacement operations instead of being treated as literal text. Includes debug logging for formula processing (January 2025).
...
- **PENDING Status Control**: Enhanced user edit modal to strictly control PENDING status - users with PENDING status have no status field or options displayed (completely hidden), while ACTIVE/INACTIVE users can only toggle between those two states. PENDING status remains exclusively system-controlled for proper user lifecycle management (January 2025).
- **Flow Transfer Document Junction Control**: Implemented conditional document-flow execution association functionality in Flow Diagram Modal. When endNode "Transferência para Fluxo" is executed, the system checks the node's `docTransfer` property: if TRUE, creates junction records in `documentflowexec_docedition` table linking the new flow execution to existing document editions; if FALSE, skips junction creation. This allows fine-grained control over document-flow relationships during transfer operations (January 2025).
- **User Access Tags Display**: Added "Acessos" column to Users table between Email and Status columns, displaying user's flowProcessAcs tags as blue badges. Shows "Sem acessos" message when no access tags are assigned. Includes responsive layout with flex-wrap for multiple tags and consistent dark/light mode styling (January 2025).
