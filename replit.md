...
- **Action Node Approval Processing**: Enhanced action nodes of type "Intern_Aprove" with specialized approval workflow. When approval status is "SIM" (TRUE), the system automatically sets document_editions.status to "ready_to_next", updates document taskState accordingly, and displays an orange "[Encaminhado]" badge. Documents remain visible in the composer throughout the approval process.
